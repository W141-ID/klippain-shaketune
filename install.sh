...
function check_download {
    ...
    if [ ! -d "${K_SHAKETUNE_PATH}" ]; then
        ...
        git clone --branch swap-hack https://github.com/W141-ID/klippain-shaketune.git "${K_SHAKETUNE_PATH}"
        if [ $? -eq 0 ]; then
            chmod +x ${K_SHAKETUNE_PATH}/install.sh
            printf "[DOWNLOAD] Download complete!\n\n"
        else
            echo "[ERROR] Download of Klippain Shake&Tune module repository failed!"
            exit -1
        fi
    else
...
case $answer1 in
    [yY][eE][sS]|[yY])
        ...
        ;;
    *)
        # Ask the user if they still want to restart Klipper and Moonraker
